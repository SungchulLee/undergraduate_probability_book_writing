...
## Marginal Distribution Problems
...
**Exercise 6.5.** Suppose $P(X = x) = 1/3$ for $x \in \{0, 1, 2\}$ and the conditional distribution of $Y$ given $X = x$ is $\text{B}(x, 0.5)$. Reconstruct the joint PMF of $(X, Y)$.
...
**Exercise 6.7.** Can two different joint distributions have the same pair of marginal distributions? If yes, provide an example. If no, explain why not.

**Exercise 6.8.** Explain why knowing only the two marginal distributions is insufficient to determine the joint distribution, unless independence is assumed.
